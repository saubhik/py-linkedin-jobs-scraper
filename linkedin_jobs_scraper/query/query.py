...
class __Base:
    def __str__(self):
        params = [
            ...
            if v is not None and not self.__is_empty_list(v)
        ]
...
class QueryFilters(__Base):
    ...
    def validate(self):
        if self.company_jobs_url is not None:
            ...
            try:
                ...
                if "f_C" not in query_params:
                    raise ValueError(
                        "Parameter company_jobs_url is invalid. "
        ...
        if self.relevance is not None and not isinstance(
            self.relevance, RelevanceFilters
        ):
        ...
        if any((not isinstance(e, TypeFilters) for e in self.type)):
            raise ValueError(
                "Parameter type must be of type Union[TypeFilters, List[TypeFilters]]"
            )

        if any((not isinstance(e, ExperienceLevelFilters) for e in self.experience)):
            raise ValueError(
                "Parameter experience must be of type "
                "Union[ExperienceLevelFilters, List[ExperienceLevelFilters]]"
            )

        if any(
            (not isinstance(e, OnSiteOrRemoteFilters) for e in self.on_site_or_remote)
        ):
            raise ValueError(
                "Parameter on_site_or_remote must be of type "
    ...
    def validate(self):
        ...
        if self.locations is not None:
            if not isinstance(self.locations, List) or any(
                [not isinstance(e, str) for e in self.locations]
            ):
        ...
        if self.skip_promoted_jobs is not None and not isinstance(
            self.skip_promoted_jobs, bool
        ):
...
class Query(__Base):
    ...
    def merge_options(self, options: QueryOptions):
        ...
        if self.options.apply_link is None:
            self.options.apply_link = (
                options.apply_link if options.apply_link is not None else False
            )

        if self.options.skip_promoted_jobs is None:
            self.options.skip_promoted_jobs = (
                options.skip_promoted_jobs
                if options.skip_promoted_jobs is not None
                else False
            )
